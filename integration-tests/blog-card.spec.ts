...
test.describe('Blog Cards', () => {
  test('displays blog cards with all required elements', async ({ page }) => {
    ...
    // Verify blog cards are displayed (should be 4 total: 1 featured + 3 regular cards)
    await expect(blogPage.blogGrid.blogCards).toHaveCount(3) // Regular cards only (excluding featured)
